...
SOURCES = {"AI圈": [{"url":"https://36kr.com/feed","type":"rss"},{"url":"https://www.ithome.com/rss/","type":"rss"}],"手机数码": [{"url":"https://www.ithome.com/rss/","type":"rss"}],"民生大事": [{"url":"https://www.thepaper.cn/rss/news.xml","type":"rss"}],"国际大事": [{"url":"https://www.huanqiu.com/rss/all.xml","type":"rss"}],"股票金融": [{"url":"https://finance.eastmoney.com/rss/fund.xml","type":"rss"}],"爆火短视频": [{"url":"https://api.bilibili.com/x/web-interface/popular?ps=10","type":"bili"}]}
...
def main():
    ...
    for sec, srcs in SOURCES.items():
        ...
        icon = ICONS.get(sec,"")
        secs += '<div style="background:'+c+';color:#fff;padding:10px 14px;border-radius:6px;font-size:15px;font-weight:bold;margin:16px 0 10px 0;">'+icon+' '+sec+'</div>'
        if not all_items:
            ...
            for item in all_items[:3]:
                img = ""
                if item.get("i"):
                    img = '<img src="'+escape(item["i"])+'" style="width:100%;height:140px;object-fit:cover;border-radius:6px;margin-bottom:8px;">'
                video = ""
                if item.get("v"):
    ...
    html = '<!DOCTYPE html><html><head><meta charset=UTF-8><meta name=viewport content="width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no"><style>*{box-sizing:border-box}body{margin:0;padding:0;background:#f0f2f5;font-family:-apple-system,\"PingFang SC\",\"Microsoft YaHei\",sans-serif}.wrap{max-width:640px;margin:0 auto;background:#fff;min-height:100vh}.hd{background:linear-gradient(135deg,#1a73e8 0%,#0d47a1 100%);padding:36px 20px 22px;text-align:center;color:#fff}.hd .t{font-size:21px;font-weight:bold;letter-spacing:1px;margin:0}.hd .d{font-size:12px;color:rgba(255,255,255,0.8);margin-top:6px}.ct{padding:14px}.ft{border-top:1px solid #e0e0e0;padding:16px;text-align:center;font-size:11px;color:#bbb}@media(max-width:480px){.hd{padding:28px 14px 18px}.hd .t{font-size:18px}.ct{padding:10px}}</style></head><body><div class=wrap><div class=hd><p class=t>📰 每日资讯简报</p><p class=d>'+ds+' | '+period+' | '+SENDER+'</p></div><div class=ct>'+secs+'</div><div class=ft>资讯来源均为权威媒体，仅供参考。投资有风险，入市需谨慎。</div></div></body></html>'
